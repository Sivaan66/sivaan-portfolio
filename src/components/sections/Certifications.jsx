...
function CertificationModal({ cert, onClose }) {
  ...
  const certificateUrl = getCertificateUrl(cert.certificateFile);
  const isPdf = cert.certificateMime === "application/pdf";
      ...
      <div className="glass-panel bg-surface-panel w-full max-w-5xl relative p-5 sm:p-7 my-2 sm:my-4" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="grid lg:grid-cols-[1.25fr_0.75fr] gap-4">
          <div className="rounded-xl border border-surface-border bg-surface-panel/60 p-3 sm:p-4">
            <div className="flex items-center justify-between gap-2 mb-3">
              ...
              {certificateUrl && <a href={certificateUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-md border border-signal/30 bg-signal/5 px-2.5 py-1.5 font-mono text-[8px] text-signal hover:bg-signal/10 transition-colors">Open certificate <ExternalLink size={11} /></a>}
            </div>
            {certificateUrl ? (
              isPdf ? (
                <div className="overflow-hidden rounded-lg border border-surface-border bg-black/20">
                  <iframe src={`${certificateUrl}#toolbar=1&navpanes=0&view=FitH`} title={`${cert.title} certificate PDF`} className="w-full h-[62vh] min-h-[420px] sm:min-h-[560px]" />
                </div>
              ) : (
                <div className="overflow-auto rounded-lg border border-surface-border bg-black/20 p-2">
                  <img src={certificateUrl} alt={`${cert.title} certificate`} className="block w-full h-auto rounded-md" />
                </div>
              )
            ) : (
        ...
        <div className="mt-5 pt-4 border-t border-surface-border flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {certificateUrl && <a href={certificateUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-md border border-surface-border bg-surface-raised/40 px-3 py-2 font-mono text-[9px] text-ink-muted hover:text-ink transition-colors"><FileText size={12} /> View full certificate</a>}
